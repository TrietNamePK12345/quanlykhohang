                    ...
                    <div class="col-sm-12">
                        <div class="card card-table">
                            ...
                            <div class="title-header option-title d-sm-flex d-block ">
                                ...
                                <button type="submit" class="btn btn-theme ">Tạo đơn hàng</button>
                            </div>
                        </div>
                    </div>
                    <!-------- step 1 -------->
                                ...
                                <div class="card card-table">
                                    <div class="card-body">
                                        ...
                                        <div class="mb-4 row align-items-center mt-3">
                                            <div class="col-sm-12">
                                                <select class="form-control w-100" name="supplier_id" required>
                                                    <option value="" selected hidden>Chọn nhà cung cấp</option>
                                                    @foreach($listSupplier as $lSup)
                                                    <option value="{{$lSup['supplier_id']}}">{{$lSup['name']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-4 row align-items-center mt-3">
                                            <div class="col-sm-12" id="filter">
                                                <select  class="form-select" name="product_id" id="select-product" required>
                                                    <option value="" selected hidden>Chọn sản phẩm</option>
                                                    @foreach($listProduct as $lProduct)
                                                    <option value="{{$lProduct['product_id']}}">{{$lProduct['name']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                            ...
                            <div class="col-sm-4">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-4 row align-items-center mt-3">
                                            <div class="col-sm-12">
                                                ...
                                                <input type="text" name="import_id" class="form-control">
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-4 row align-items-center mt-3">
                                            <div class="col-sm-12">
                                                ...
                                                <select class="form-control w-100" name="warehouse_id" required>
                                                    <option value="" selected hidden>Chọn địa chỉ</option>
                                                    @foreach($listWarehouse as $lWarehouse)
                                                    <option value="{{$lWarehouse['warehouse_id']}}">{{$lWarehouse['address']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="card card-table">
                                    <div class="card-body">
                                        ...
                                        <div class="mb-4 row align-items-center mt-3">
                                            <div class="col-sm-12">
                                                <textarea class="form-control" name="note" id="" cols="50" rows="8" required></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    $(document).ready(function() {
        $("#select-product").on('change', function() {
            ...
            $.ajax({
                url: '/tim-san-pham', // Assuming the correct URL endpoint
                method: 'POST',
                data: { id: productId },
                dataType: "JSON",
                success: function(data) {
                    // Update product information on the interface
                    if (data) { // Check if data is valid
                        var $infoProduct = $("#infoProduct");
                        $infoProduct.empty(); // Clear existing content

                        // Create the product info table row
                        var productRow =
                            "<td>" + data.product_id + "</td>" +
                            "<td><div class='table-image'><img class='img-fluid' src='" + data.image + "'></div></td>" +
                            "<td>" + data.name + "</td>" +
                            "<td><input type='number' min='10' name='qty_buy' class='quantity-input' required></td>" +
                            "<td><input type='number' min='10' name='import_price' class='price-input' required></td>" +
                            "<td><input type='text' disabled name='totalPrice' class='total-price' required> VNĐ</td>"

                        $infoProduct.append(productRow);

                        $('.quantity-input, .price-input').on('change', function() {
                            var $quantity = $(this).closest('tr').find('.quantity-input');
                            var $price = $(this).closest('tr').find('.price-input');
                            var $totalPrice = $(this).closest('tr').find('.total-price');

                            var quantity = Math.max(0, parseInt($quantity.val()));
                            var price = Math.max(0, parseFloat($price.val()));

                            var totalPrice = quantity * price;

                            if (!isNaN(totalPrice)) {
                                $totalPrice.val(totalPrice.toLocaleString('vi-VN'));
                            }
                        });
                    } else {

                        console.error("Lỗi khi tìm nạp thông tin sản phẩm");
                    }
                },
